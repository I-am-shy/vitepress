...
| io.of('/namespace').on('connection', (socket) => {}) | 监听命名空间连接，表示连接到 `localhost:3000/namespace` ，默认为 `/` 。 |
| socket.emit('event', ...args) | 触发事件，可以传递多个参数（任意类型，任意数量），参数会传递给事件监听函数。 |
| socket.on('event', (...args) => {}) | 监听事件。args 为 emit 传递的参数。 |
