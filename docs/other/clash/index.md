...
>[!NOTE] 注意
> Trojan 节点启动时需要使用 80 端口，若使用的服务器中有其他服务（如 nginx）占用了 80 端口，需要先停止其他服务，或者修改其他服务的端口。
...
安装后会自动进行配置选项并启动 v2ray 服务，配置完成后会显示 v2ray 的配置信息，此时 v2ray 节点搭建完成。

1. 传输协议：选 1. TCP (最稳定，新手推荐)。
2.端口：直接按回车，使用随机端口。
3.屏蔽广告：直接按回车。
4.Shadowsocks配置：回车选 N。

安装完成后，屏幕会显示一串核心信息（IP、端口、UUID等），这是你自建节点的“账号密码”，用于客户端配置。
...
```yaml {6,7,8}
# V2Ray
# 核心：你的 VMess-TCP 节点配置（已填充所有服务端信息）
proxies:
  - name: "VMess-TCP-节点"  # 自定义节点名，可随意改（如"阿里云-VMess-TCP"）
    type: vmess            # 协议类型，固定为 vmess
    server: xx.xx.xx.xx  # 服务端 IP，你的实际地址
    port: xx            # 服务端端口，你的实际端口
    uuid: xx-xx-xx-xx-xx  # 服务端用户ID，一字不能错
    alterId: 0             # 额外ID，新版VMess默认0，与服务端匹配
    cipher: auto           # 加密方式，auto 自动适配，无需手动指定
    network: tcp           # 传输协议，固定为 tcp（对应VMess-TCP）
    tls: false             # 你的服务端未开启TLS，固定为false
    servername: ""         # 无TLS则留空，无需填写
    skip-cert-verify: false # 无TLS，此配置无效，保持false即可
    # TCP 优化配置，默认开启即可
    tcp-opts:
      fast-open: true      # 启用TCP快速打开，提升连接速度
      keep-alive: true     # 保持TCP长连接，减少重连次数
...
- [Clash 代理服务器自建节点一键搭建 - Trojan 搭建教程](https://clashyun.com/264.html)
- [v2ray 节点搭建](https://clashxhub.com/banwagon-v2ray/)