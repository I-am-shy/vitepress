...
```bash
# for，mv 批量修改文件名
# 匹配当前目录下所有文件名包含 @2x. 的文件，并把文件名中的 @2x 字符串替换为空（即删除）
for f in *@2x.*; do new="${f/@2x/}"; echo "Renaming: $f -> $new"; mv "$f" "$new"; done

```

- `for f in *@2x.*;`：精准匹配当前目录下所有文件名包含 @2x. 的文件。
- `"${f/@2x/}"`：把文件名中的 @2x 字符串替换为空（即删除）。
- `mv`：执行重命名。
